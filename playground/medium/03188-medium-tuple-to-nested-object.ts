...
type STest<T> =  T extends `${infer T}${infer Rest}`  ? Rest :false
type S = STest<'a'>
type MTest<T> =  T extends [infer First extends string, ...infer Rest ]  ? Rest :false
type A = MTest<['a']> 
...
type MyTest = TupleToNestedObject<['a', 'b', 'c'], boolean>
