...
type StringExclude<S, C, A extends string = ''> = S extends `${infer F}${infer Rest}`
  ? F extends C 
    ? `${A}${Rest}`
    : StringExclude<Rest, C, `${A}${F}`>
  : A

type SETest =  StringExclude<'ABC', 'B'>
...
> = S extends `${infer F}${infer R}`
  ? U extends U
    ? `${U}${AllCombinations<R, U extends ''
       ? T // pool sin cambios
       : Exclude<T, U>>}`
    : never
  : ''

type MT = AllCombinations<'A'>
