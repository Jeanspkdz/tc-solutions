...
type DropChar<S, C extends string, Acc extends string = ''> = 
S extends `${infer Match extends C}${infer Rest}` 
  ? DropChar<Rest, C, Acc> 
  : S extends `${infer Char}${infer Rest}` 
    ? DropChar<Rest, C, `${Acc}${Char}`>
    : Acc

type MyTest = DropChar<'butter fly!', 'f'>
